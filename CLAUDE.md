...
- The `screen` subcommand (`screen_label_criteria`/`summarize_screen`) is a *necessary-condition* check: per-year mean return of all stocks meeting a criterion = the precision-1/recall-1 portfolio. Criterion specs: plain numbers are absolute thresholds, `bmk`±offset is benchmark-relative (`parse_criterion`). It deliberately runs no selection simulation; don't wire it into the sweep loop.
