...
- Tests live in `tests/` as plain-assert scripts (no pytest dependency): `uv run python tests/test_year_universe_returns.py`. A full `pickn.py` run needs network access to yfinance and can take a long time uncached; prefer testing the pure functions (`simulate_model.py`, `year_universe_returns`) when validating changes.
